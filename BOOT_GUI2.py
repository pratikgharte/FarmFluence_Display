...
def read_sensor():
    ...
    while True:
       ...
       # Temp = temp_val
        #Tempgate = Temp
        temp1 = "{:.1f}".format(temp_val)
        #temp2 = int(temp1)
        #print(temp1)
        
        
        #pH =  int(float(format_float))
        #format_float1 = "{:.0f}".format(ec)
        format_float1 = "{:.0f}".format(ec_val)
        
        format_float = "{:.1f}".format(ph_val)
        #p5 = ph_val
        #pH4 = int(p5)
        #if(pH4 > 14):
            #format_float = 6.5
        ...
        var1.set("{}".format(format_float1))
        var20.set("{}".format(temp1))
        print("EC:" + str(format_float1), "Temperature:" + str(temp1)  # print the readings
              + "\t PH:" + str(format_float))
        ECT = ec_val
        ECT2=ECT        
...
class MainScreen:
    ...
    def menu_Screen(self):
        ...
        empty_Btn=Button(menu_C,image=emptBtn,command=lambda: start_Cal10_Mode(var5),relief=FLAT,  ##Back to main screen
                        fg='BLACK',bg='#F7F7F7',bd=0,highlightthickness=0,borderwidth=0)
